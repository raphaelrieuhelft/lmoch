...
*)
  ...
  try
	...
	let proof_fname = (Filename.chop_suffix (Filename.basename file) ".lus")^("_proof.ml") in
	let proof_path = proof_dir^"/"^proof_fname in
	Format.printf "proof path : %s@." proof_path;
	let rm_command = "rm "^proof_path in
	ignore (Unix.system rm_command);
	let command = "cp "^kind_solver^" "^proof_path in
	ignore (Unix.system command);
	let fd = Unix.openfile proof_path [Unix.O_RDWR; Unix.O_APPEND] 0o640 in
	ignore (Unix.lseek fd 0 Unix.SEEK_END);
	let oc = Unix.out_channel_of_descr fd in
	let ff = Format.formatter_of_out_channel oc in
	Format.fprintf ff "(*Beginning autogenerated proof*) @.@.";
	let () = Code_generation.main ff decls input_tvars output_id in
	Unix.close fd;
    ...
    Format.printf "Proof printed in %s, compile and run that file or use -run-proof option for results.@." proof_path);
